...
public class WorldGenAcademy implements IWorldGenerator
{
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
    {
        ...
        for (ScanY = 2; ScanY < Layers; ScanY++)
        {
            ...
            {
                if (!world.isAirBlock(chunkX - WidthX / 2, posY + ScanY, chunkZ - WidthZ / 2))
                {
                    return;
                }
            }
        ...
        for (ScanY = 0; ScanY < Layers; ScanY++)
        {
            for (ScanZ = 0; ScanZ < WidthZ; ScanZ++)
            {
                for (ScanX = 0; ScanX < WidthX; ScanX++)
                {
                    ...
                    if (TmpBlock == Block.glowStone.blockID && random.nextInt(100) < 10)
                    {
                        TmpBlock = 0;
                    }
                    ...
                    {
                        if (TmpBlock == Block.blockClay.blockID)
                        {
                            TmpMD = 2;
                        }
    ...
    public ItemStack GetRandomContent(Random rand, boolean isDisc)
    {
        ...
        if (isDisc)
        {
            return (new ItemStack(Item.itemsList[Item.record13.shiftedIndex + rand.nextInt(11)], 1));
        }
        else
        {
            if (Chance < 10)
