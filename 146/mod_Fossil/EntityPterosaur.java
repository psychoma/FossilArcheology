...
public class EntityPterosaur extends EntityDinosaurce
{
    ...
    protected void attackEntity(Entity entity, float f)
    {
        ...
        else if ((double)f < 1.9F && entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY < boundingBox.maxY)
        {
            attackTime = 20;
    ...
    public void updateRiderPosition()
    {
        ...
        else
        {
            if (this.onGround)
            {
                riddenByEntity.setPosition(posX, posY - YAxieOffset * 1.1, posZ);
            }
            else if (this.Landing)
            {
                riddenByEntity.setPosition(posX, posY - YAxieOffset, posZ);
            }
            else
            {
                riddenByEntity.setPosition(posX, posY - YAxieOffset * 0.6, posZ);
            }
    ...
    public boolean interact(EntityPlayer entityplayer)
    {
        ...
        if (isTamed())
        {
            if (itemstack != null)
            {
                ...
                if (EOCInteract(itemstack, entityplayer))
                {
                    return true;
                }
...
//                        entityplayer.moveForward = 0;
    ...
    public void ShowPedia(EntityPlayer checker)
    {
        ...
        if (this.isTamed())
        {
            ...
            {
                mod_Fossil.ShowMessage(new StringBuilder().append(FlyText).toString(), checker);
            }

            if (this.getDinoAge() >= 8)
            {
                mod_Fossil.ShowMessage(new StringBuilder().append(RidiableText).toString(), checker);
            }
        }
        else
        {
    ...
    private void HandleRiding()
    {
        ...
        if (this.onGround || this.inWater)
        {
            ...
            this.rotationYaw -= (float)((float)(ridder.movementInput.moveStrafe) * 5);

            for (; this.rotationYaw < -180F; this.rotationYaw += 360F) { }

            for (; this.rotationYaw >= 180F; this.rotationYaw -= 360F) { }

            this.setMoveForward(ridder.movementInput.moveForward * this.moveSpeed);
        }
        else
        {
            this.AirAngle -= ridder.movementInput.moveStrafe;
            ...
            {
                this.rotationYaw += (this.AirAngle > 0 ? 1 : -1);
            }
            ...
            else
            {
                ...
                this.AirPitch -= ridder.movementInput.moveForward * 2;
    ...
    public EntityPterosaur spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
