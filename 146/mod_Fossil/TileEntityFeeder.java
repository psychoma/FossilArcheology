    ...
    implements IInventory, ISidedInventory
{
    ...
    public void updateEntity()
    {
            ...
            if (MeatValue(target) > 0)
            {
                ...
                else
                {
                    while ((MeatValue(target) + MeatCurrent < MeatMax) && feederItemStacks[0] != null)
                    {
                        ...
                        if (feederItemStacks[0].stackSize == 0)
                        {
                            feederItemStacks[0] = null;
                        }
                    }
            ...
            if (VegValue(target) > 0)
            {
                ...
                else
                {
                    while ((VegValue(target) + VegCurrent < VegMax) && feederItemStacks[1] != null)
                    {
                        ...
                        if (feederItemStacks[1].stackSize == 0)
                        {
                            feederItemStacks[1] = null;
                        }
                    }
        ...
        {
            BlockFeeder.updateFurnaceBlockState(flag_After, worldObj, xCoord, yCoord, zCoord);
        }
    ...
    {
        if (target == Item.egg.shiftedIndex)
        {
            return 100;
        }

        if (target == Item.porkRaw.shiftedIndex)
        {
            return 20;
        }

        if (target == Item.porkCooked.shiftedIndex)
        {
            return 30;
        }

        if (target == Item.chickenCooked.shiftedIndex)
        {
            return 10;
        }

        if (target == Item.chickenRaw.shiftedIndex)
        {
            return 30;
        }

        if (target == Item.beefCooked.shiftedIndex)
        {
            return 20;
        }

        if (target == Item.beefRaw.shiftedIndex)
        {
            return 40;
        }

        if (target == Item.fishRaw.shiftedIndex)
        {
            return 40;
        }

        if (target == Item.fishCooked.shiftedIndex)
        {
            return 60;
        }

        if (target == Item.slimeBall.shiftedIndex)
        {
            return 10;
        }

        if (target == mod_Fossil.RawDinoMeat.shiftedIndex)
        {
            return 100;
        }

        if (target == mod_Fossil.CookedDinoMeat.shiftedIndex)
        {
            return 100;
        }
    ...
    {
        if (target == mod_Fossil.FernSeed.shiftedIndex)
        {
            return 50;
        }

        if (target == Item.appleRed.shiftedIndex)
        {
            return 100;
        }

        if (target == Item.wheat.shiftedIndex)
        {
            return 40;
        }

        if (target == Item.reed.shiftedIndex)
        {
            return 20;
        }

        if (target == Item.paper.shiftedIndex)
        {
            return 60;
        }

        if (target == Item.book.shiftedIndex)
        {
            return 180;
        }

        if (target == Item.bread.shiftedIndex)
        {
            return 120;
        }

        if (target == Item.melon.shiftedIndex)
        {
            return 25;
        }

        if (target == Block.plantRed.blockID || target == Block.plantYellow.blockID)
        {
            return 20;
        }

        if (target == Block.mushroomBrown.blockID || target == Block.mushroomBrown.blockID)
        {
            return 15;
        }

        if (target == Block.leaves.blockID || target == Block.vine.blockID)
        {
            return 10;
        }
    ...
    {
        if (eating == EnumDinoEating.Herbivorous)
        {
