...
//        super.render(f,f1,f2, f3, float f4, float f5)
    ...
    {
        if (noumenon_1.rotationPointY > 7)
        {
            noumenon_1.rotationPointY -= (2.0F / moves);
        }
        else
        {
            noumenon_1.rotationPointY = 7;
        }

        if (noumenon_1.rotationPointZ > -4)
        {
            noumenon_1.rotationPointZ -= (3.0F / moves);
        }
        else
        {
            noumenon_1.rotationPointZ = -4;
        }

        if (noumenon_1.rotateAngleX < 0)
        {
            noumenon_1.rotateAngleX += (0.9948377F / moves);
        }
        else
        {
            noumenon_1.rotateAngleX = 0;
        }

        if (noumenon_2.rotationPointZ > 0)
        {
            noumenon_2.rotationPointZ -= (2.0F / moves);
        }
        else
        {
            noumenon_2.rotationPointZ = 0;
        }

        if (noumenon_2.rotateAngleX < 0)
        {
            noumenon_2.rotateAngleX += (0.4068249F / moves);
        }
        else
        {
            noumenon_2.rotateAngleX = 0;
        }

        if (hand_R.rotationPointY < 14)
        {
            hand_R.rotationPointY += (4.0F / moves);
        }
        else
        {
            hand_R.rotationPointY = 14;
        }

        if (hand_L.rotationPointY < 14)
        {
            hand_L.rotationPointY += (4.0F / moves);
        }
        else
        {
            hand_L.rotationPointY = 14;
        }

        if (noumenon_3.rotateAngleX < -0.2260139F)
        {
            noumenon_3.rotateAngleX += ((0.7684471F - 0.2260139F) / moves);
        }
        else
        {
            noumenon_3.rotateAngleX = -0.2260139F;
        }

        if (noumenon_4.rotationPointY > 11)
        {
            noumenon_4.rotationPointY -= (4.0F / moves);
        }
        else
        {
            noumenon_4.rotationPointY = 11;
        }

        if (noumenon_4.rotationPointZ < 14)
        {
            noumenon_4.rotationPointZ += (2.0F / moves);
        }
        else
        {
            noumenon_4.rotationPointZ = 14;
        }

        if (noumenon_4.rotateAngleX < -0.1356083F)
        {
            noumenon_4.rotateAngleX += ((0.5424333F - 0.1356083F) / moves);
        }
        else
        {
            noumenon_4.rotateAngleX = -0.1356083F;
        }

        if (noumenon_5.rotationPointY > 13)
        {
            noumenon_5.rotationPointY -= (7.0F / moves);
        }
        else
        {
            noumenon_5.rotationPointY = 13;
        }

        if (noumenon_5.rotationPointZ < 22)
        {
            noumenon_5.rotationPointZ += (3.0F / moves);
        }
        else
        {
            noumenon_5.rotationPointZ = 22;
        }

        if (noumenon_5.rotateAngleX < 0)
        {
            noumenon_5.rotateAngleX += (0.3616222F / moves);
        }
        else
        {
            noumenon_5.rotateAngleX = 0;
        }

        if (head_1.rotationPointY < 7)
        {
            head_1.rotationPointY += (7.0F / moves);
        }
        else
        {
            head_1.rotationPointY = 7;
        }

        if (head_1.rotationPointZ > -14)
        {
            head_1.rotationPointZ -= (6.0F / moves);
        }
        else
        {
            head_1.rotationPointZ = -14;
        }
        ...
        if (head_2.rotationPointY < 7)
        {
            head_2.rotationPointY += (7.0F / moves);
        }
        else
        {
            head_2.rotationPointY = 7;
        }

        if (head_2.rotationPointZ > -14)
        {
            head_2.rotationPointZ -= (6.0F / moves);
        }
        else
        {
            head_2.rotationPointZ = -14;
        }

        //if (head_2.rotateAngleX>-0.1356083F) head_2.rotateAngleX-=(0.1356083F/moves);
        //else head_2.rotateAngleX=-0.1356083F;

        if (head_3.rotationPointY < 7)
        {
            head_3.rotationPointY += (6.0F / moves);
        }
        else
        {
            head_3.rotationPointY = 7;
        }

        if (head_3.rotationPointZ > -14)
        {
            head_3.rotationPointZ -= (6.0F / moves);
        }
        else
        {
            head_3.rotationPointZ = -14;
        }
    ...
    {
        if (noumenon_1.rotationPointY < 9)
        {
            noumenon_1.rotationPointY += (2.0F / moves);
        }
        else
        {
            noumenon_1.rotationPointY = 9;
        }

        if (noumenon_1.rotationPointZ < -1)
        {
            noumenon_1.rotationPointZ += (3.0F / moves);
        }
        else
        {
            noumenon_1.rotationPointZ = -1;
        }

        if (noumenon_1.rotateAngleX > -0.9948377F)
        {
            noumenon_1.rotateAngleX -= (0.9948377F / moves);
        }
        else
        ...
        if (noumenon_2.rotationPointZ < 2)
        {
            noumenon_2.rotationPointZ += (2.0F / moves);
        }
        else
        {
            noumenon_2.rotationPointZ = 2;
        }

        if (noumenon_2.rotateAngleX > -0.4068249F)
        {
            noumenon_2.rotateAngleX -= (0.4068249F / moves);
        }
        else
        ...
        if (hand_R.rotationPointY > 10)
        {
            hand_R.rotationPointY -= (4.0F / moves);
        }
        else
        {
            hand_R.rotationPointY = 10;
        }

        if (hand_L.rotationPointY > 10)
        {
            hand_L.rotationPointY -= (4.0F / moves);
        }
        else
        {
            hand_L.rotationPointY = 10;
        }

        if (noumenon_3.rotateAngleX > -0.7684471F)
        {
            noumenon_3.rotateAngleX -= ((0.7684471F - 0.2260139F) / moves);
        }
        else
        ...
        if (noumenon_4.rotationPointY < 15)
        {
            noumenon_4.rotationPointY += (4.0F / moves);
        }
        else
        {
            noumenon_4.rotationPointY = 15;
        }

        if (noumenon_4.rotationPointZ > 12)
        {
            noumenon_4.rotationPointZ -= (2.0F / moves);
        }
        else
        {
            noumenon_4.rotationPointZ = 12;
        }

        if (noumenon_4.rotateAngleX > -0.5424333F)
        {
            noumenon_4.rotateAngleX -= ((0.5424333F - 0.1356083F) / moves);
        }
        else
        ...
        if (noumenon_5.rotationPointY < 20)
        {
            noumenon_5.rotationPointY += (7.0F / moves);
        }
        else
        {
            noumenon_5.rotationPointY = 20;
        }

        if (noumenon_5.rotationPointZ > 19)
        {
            noumenon_5.rotationPointZ -= (3.0F / moves);
        }
        else
        {
            noumenon_5.rotationPointZ = 19;
        }

        if (noumenon_5.rotateAngleX < -0.3616222F)
        {
            noumenon_5.rotateAngleX += ((0.3616222F - 2.8368E-15F) / moves);
        }
        else
        {
            noumenon_5.rotateAngleX = -0.3616222F;
        ...
        if (head_1.rotationPointY > 0)
        {
            head_1.rotationPointY -= (7.0F / moves);
        }
        else
        {
            head_1.rotationPointY = 0;
        }

        if (head_1.rotationPointZ < -8)
        {
            head_1.rotationPointZ += (6.0F / moves);
        }
        else
        {
            head_1.rotationPointZ = -8;
        }

        //if (head_1.rotateAngleX>0) head_1.rotateAngleX-=(0.2260139F/moves);
        //else head_1.rotateAngleX=0;

        if (head_2.rotationPointY > 1)
        {
            head_2.rotationPointY -= (7.0F / moves);
        }
        else
        {
            head_2.rotationPointY = 1;
        }

        if (head_2.rotationPointZ < -8)
        {
            head_2.rotationPointZ += (6.0F / moves);
        }
        else
        {
            head_2.rotationPointZ = -8;
        }

        //if (head_2.rotateAngleX<0) head_2.rotateAngleX+=(0.1356083F/moves);
        //else head_2.rotateAngleX=0;

        if (head_3.rotationPointY > 1)
        {
            head_3.rotationPointY -= (6.0F / moves);
        }
        else
        {
            head_3.rotationPointY = 1;
        }

        if (head_3.rotationPointZ < -8)
        {
            head_3.rotationPointZ += (6.0F / moves);
        }
        else
        {
            head_3.rotationPointZ = -8;
        }
    ...
    {
        if (head_1.rotateAngleX < 0.2260139F)
        {
            head_1.rotateAngleX += (0.2260139F / moves);
        }
        else
        {
            head_1.rotateAngleX = 0.2260139F;
        }

        if (head_2.rotateAngleX > -0.1356083F)
        {
            head_2.rotateAngleX -= (0.1356083F / moves);
        }
        else
        {
            head_2.rotateAngleX = -0.1356083F;
        }

        if (head_3.rotateAngleX < 0.4520277F)
        {
            head_3.rotateAngleX += (0.4520277F / moves);
        }
        else
        {
            head_3.rotateAngleX = 0.4520277F;
        }
    }
    public void CloseMouth(float moves)
    {
        if (head_1.rotateAngleX > 0)
        {
            head_1.rotateAngleX -= (0.2260139F / moves);
        }
        else
        {
            head_1.rotateAngleX = 0;
        }

        if (head_2.rotateAngleX < 0)
        {
            head_2.rotateAngleX += (0.1356083F / moves);
        }
        else
        {
            head_2.rotateAngleX = 0;
        }

        if (head_3.rotateAngleX > 0)
        {
            head_3.rotateAngleX -= (0.4520277F / moves);
        }
        else
        {
            head_3.rotateAngleX = 0;
        }
    }
    public boolean SwingHeadLeft(int step)
    {
        ...
        if (head_3.rotateAngleZ > (-this.SwingAngle))
        {
            head_3.rotateAngleZ -= (this.SwingAngle / step);
        }
        else
        {
            StepCount++;
        ...
        if (head_2.rotateAngleZ > (-this.SwingAngle))
        {
            head_2.rotateAngleZ -= (this.SwingAngle / step);
        }
        else
        {
            StepCount++;
        ...
        if (head_1.rotateAngleZ > (-this.SwingAngle))
        {
            head_1.rotateAngleZ -= (this.SwingAngle / step);
        }
        else
        {
            StepCount++;
...
//fields
            ...
            float f4, float f5, boolean modelized)
    {
        ...
        //HeadYaw
        head_1.rotateAngleY = -f3 / (180F / (float)Math.PI);
        head_2.rotateAngleY = -f3 / (180F / (float)Math.PI);
        head_3.rotateAngleY = -f3 / (180F / (float)Math.PI);
        //right leg
        ...
        //left leg
        foot_L_1.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1;
        foot_L_2.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1 - 0.6108652F;
        foot_L_3.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1;
