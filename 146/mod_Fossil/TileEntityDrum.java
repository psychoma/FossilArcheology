...
public class TileEntityDrum extends TileEntity
{
    ...
    public boolean SendOrder(int UsingID, EntityPlayer player)
    {
        ...
        if (UsingID != Item.stick.shiftedIndex && UsingID != Item.bone.shiftedIndex && UsingID != mod_Fossil.SkullStick.shiftedIndex && UsingID != Item.arrow.shiftedIndex)
        {
            return false;
        }
    ...
    {
        List list = worldObj.getEntitiesWithinAABB(EntityRaptor.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(xCoord, yCoord, zCoord, xCoord + 1.0D, yCoord + 1.0D, zCoord + 1.0D).expand(30D, 4D, 30D));
    ...
    {
        List list = worldObj.getEntitiesWithinAABB(EntityPterosaur.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(xCoord, yCoord, zCoord, xCoord + 1.0D, yCoord + 1.0D, zCoord + 1.0D).expand(30D, 4D, 30D));
        Iterator iterator = list.iterator();
    ...
    {
        List list = worldObj.getEntitiesWithinAABB(EntityTriceratops.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(xCoord, yCoord, zCoord, xCoord + 1.0D, yCoord + 1.0D, zCoord + 1.0D).expand(30D, 4D, 30D));
        Iterator iterator = list.iterator();
    ...
    {
        List list = worldObj.getEntitiesWithinAABB(EntityTRex.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(xCoord, yCoord, zCoord, xCoord + 1.0D, yCoord + 1.0D, zCoord + 1.0D).expand(50D, 4D, 50D));
        Iterator iterator = list.iterator();
...
//                entityTRex.entityToAttack = player;
                entityTRex.setAttackTarget(player);
            }
        }
        while (true);
    }
