...
public class EntityStegosaurus extends EntityDinosaurce
{
    ...
    public float getShakeAngle(float f, float f1)
    {
        ...
        return MathHelper.sin(f2 * (float)Math.PI) * MathHelper.sin(f2 * (float)Math.PI * 11F) * 0.15F * (float)Math.PI;
    }
    ...
    private boolean FindWheats(int range)
    {
        if (!isSelfSitting())
        {
            ...
            if (NearBy != null)
            {
                for (int i = 0; i < NearBy.size(); i++)
                {
                    if (NearBy.get(i) instanceof EntityItem)
                    {
                        ...
                        if (ItemInRange.func_92014_d().itemID == Item.wheat.shiftedIndex)
                        {
                            HandleEating(10);
            ...
            List searchlist = worldObj.getEntitiesWithinAABB(net.minecraft.entity.item.EntityItem.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(range, 4D, range));
            ...
            do
            {
                ...
                if (entity1 instanceof EntityItem)
                {
                    ...
                    if (Tmp.func_92014_d().getItem().shiftedIndex == Item.wheat.shiftedIndex)
                    {
                        if (TargetItem != null)
                        {
        ...
        if (riddenByEntity != null)
        {
            if (this.riddenByEntity instanceof EntityPlayerSP)
            {
                if (this.onGround)
                {
                    ...
                    {
                        if (this.getDinoAge() > 4 && (this.motionX != 0 && this.motionZ != 0))
                        {
                            BlockInteractive();
                        }
                        ...
                        if (this.RushTick == 0)
                        {
                            this.rotationYaw -= (float)((float)(((EntityPlayerSP) this.riddenByEntity).movementInput.moveStrafe) * 5);

                            for (; this.rotationYaw < -180F; this.rotationYaw += 360F) { }

                            for (; this.rotationYaw >= 180F; this.rotationYaw -= 360F) { }

                            this.moveForward = ((EntityPlayerSP) this.riddenByEntity).movementInput.moveForward * this.moveSpeed * 8;
                        }
                        else
                        {
                            this.rotationYaw -= (float)((float)(((EntityPlayerSP) this.riddenByEntity).movementInput.moveStrafe) * 5);
                            ...
                            this.moveForward = ((EntityPlayerSP) this.riddenByEntity).movementInput.moveForward * this.moveSpeed * (3 / 4);
                        }
                        ...
                        if (!this.inWater)
                        {
                            if (!this.isTamed() || this.riddenByEntity != null)
                            {
                                if (Block.blocksList[blockid].getBlockHardness(worldObj, (int)posX, (int)posY, (int)posZ) < 5.0 || blockid == Block.wood.blockID)
                                {
                                    ...
                                    {
                                        Block.blocksList[blockid].dropBlockAsItem(worldObj, j, k, l, 1, 0);
                                    }
    ...
    public EntityStegosaurus spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
