...
public class DinoAILearnChest extends EntityAIBase
{
    ...
    public DinoAILearnChest(EntityDinosaurce learnerPar)
    {
        ...
        if (learner instanceof IHighIntellegent)
        {
            learnerInterface = (IHighIntellegent)learner;
        }
    }
    @Override
    public boolean shouldExecute()
    {
        if (learner == null || learnerInterface == null)
        {
            return false;
        }

        if (learnerInterface.isLeartChest())
        {
            return false;
        }
        ...
        for (int Xoffset = -rangeX; Xoffset <= rangeX; Xoffset++)
        {
            for (int Yoffset = -rangeY; Yoffset <= rangeY; Xoffset++)
            {
                ...
                for (int Zoffset = -rangeZ; Zoffset < rangeZ; Zoffset++)
                {
                    ...
                    if (tmp instanceof TileEntityChest)
                    {
                        return (TileEntityChest) tmp;
                    }
                }
            }
        }
