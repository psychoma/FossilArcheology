...
public class ItemAncientEgg extends Item
{
    ...
    */
            ...
            else
            {
                if (var24.typeOfHit == EnumMovingObjectType.TILE)
                {
                    ...
                    if (!world.isRemote)
                    {
                        ...
                        if (!spawnCreature(world, this.GetTypeFromInt(itemstack.getItemDamage()), (double)((float)var35 + 0.5F), (double)((float)var33 + 1.0F), (double)((float)var34 + 0.5F)))
                        {
                            return itemstack;
                        }
                    }
    ...
    public static boolean spawnCreature(World par0World, EnumDinoType enumDinoType, double par2, double par4, double par6)
    {
        ...
        if (enumDinoType == EnumDinoType.Nautilus)
        {
            var8 = new EntityNautilus(par0World);
        }
        else
        {
            var8 = new EntityDinoEgg(par0World, enumDinoType);
        }
