...
public class TileEntityAnalyzer extends TileEntity implements IInventory, ISidedInventory
{
    ...
    private boolean canSmelt()
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            if (analyzerItemStacks[i] != null)
            {
                ...
                if ((tmp == mod_Fossil.biofossil.itemID) ||
                        (tmp == mod_Fossil.relic.itemID) ||
                        (tmp == mod_Fossil.RawDinoMeat.itemID) ||
                        (tmp == Item.porkRaw.itemID) ||
                        (tmp == Item.beefRaw.itemID) ||
                        (tmp == Item.egg.itemID) ||
                        (tmp == Item.chickenRaw.itemID) ||
                        (tmp == Block.cloth.blockID) ||
                        (tmp == mod_Fossil.IcedMeat.itemID)
                   )
                {
                    RawIndex = i;
                    break;
    ...
    public void smeltItem()
    {
        ...
        {
            if (new java.util.Random().nextInt(50) <= 30)
            {
                itemstack = new ItemStack(Item.silk, 4);
            }
            else
            {
                itemstack = new ItemStack(mod_Fossil.AnimalDNA, 1, 1);
            }
        }
        ...
        if (analyzerItemStacks[RawIndex].getItem() == mod_Fossil.relic)
        {
            if (chance <= 40)
            {
                itemstack = new ItemStack(Block.gravel, 6);
            }
        ...
        if (itemstack != null)
        {
            ...
            {
                if (analyzerItemStacks[SpaceIndex] == null)
                {
                    analyzerItemStacks[SpaceIndex] = itemstack.copy();
                }
