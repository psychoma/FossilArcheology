...
public class EntitySaberCat extends EntityTameable
{
    ...
    protected void updateEntityActionState()
    {
        ...
        {
            List list = worldObj.getEntitiesWithinAABB(EntityAnimal.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(16D, 4D, 16D));

            if (!list.isEmpty())
            {
                EntityAnimal Tmp = (EntityAnimal)list.get(worldObj.rand.nextInt(list.size()));

                if (!(Tmp instanceof EntitySaberCat))
                {
                    this.setAttackTarget(Tmp);
                }
            }
        }
        else
    ...
    private boolean isInterest(int ItemID)
    {
        if (isChild())
        {
            return ItemID == Item.bucketMilk.itemID;
        }
        else
        {
            return (ItemID == Item.beefRaw.itemID) ||
