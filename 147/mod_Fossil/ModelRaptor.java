...
public class ModelRaptor extends ModelBase
{
    public ModelRaptor()
    {
        ...
        noumenon4.rotateAngleX = -((float)Math.PI * 2F / 9F);
...
//super.render(float f, float f1, float f2, float f3, float f4, float f5)
    ...
    public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, boolean HeadTailFree)
    {
        if (!HeadTailFree)
        {
            ...
            //left leg
            thigh_L2.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1;
            leg_L2.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1 + 0.9948377F;
            horn_2_L.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1;
            left_nail_1.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1 - 0.8726646F;
            left_nail_2.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1 - 2.617994F;
            //right arm
            thigh_R1.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1;
            leg_R1.rotateAngleX = MathHelper.cos(f * 0.6662F + (float)Math.PI) * 1.4F * f1 + 0.994461F;
            //left arm
            ...
            head3_up.rotateAngleY = -f3 / (180F / (float)Math.PI);
            head3_down.rotateAngleY = -f3 / (180F / (float)Math.PI);*/
            if (Math.abs(thigh_L2.rotateAngleX) >= 0.174532F)
            {
                //moving
                HeadLower();
    ...
    public boolean SwingHead(float targetAngle, int step)
    {
        ...
        if (direction)
        {
            if (head2.rotateAngleZ < targetAngle)
            {
                head2.rotateAngleZ += amount;
            }
            else
            {
            ...
            if (head3_up.rotateAngleZ < targetAngle)
            {
                head3_up.rotateAngleZ += amount;
            }
            else
            {
            ...
            if (head3_down.rotateAngleZ < targetAngle)
            {
                head3_down.rotateAngleZ += amount;
            }
            else
            {
    ...
    {
        if (noumenon3.rotateAngleX < 0)
        {
            noumenon3.rotateAngleX += (0.5235988F / 10);
        }
        else
        {
        ...
        if (noumenon4.rotateAngleX < 0)
        {
            noumenon4.rotateAngleX += (((float)Math.PI * 2F / 9F) / 10);
        }
        else
        {
            noumenon4.rotateAngleX = 0F;
        }
    }
    public void TailLower()
    {
        if (noumenon3.rotateAngleX > -0.5235988F)
        {
            noumenon3.rotateAngleX -= (0.5235988F / 10);
        }
        else
        ...
        if (noumenon4.rotateAngleX > -((float)Math.PI * 2F / 9F))
        {
            noumenon4.rotateAngleX -= (((float)Math.PI * 2F / 9F) / 10);
        }
        else
    ...
    {
        if (noumenon1.rotationPointZ < -3)
        {
            noumenon1.rotationPointZ += 0.1;
        }
        else
        {
            noumenon1.rotationPointZ = -3;
        }

        if (noumenon1.rotateAngleX < 0)
        {
            noumenon1.rotateAngleX += (0.5235988F / 10);
        }
        else
        {
        ...
        if (head1.rotationPointZ > -8)
        {
            head1.rotationPointZ -= (3 / 10);
        }
        else
        {
            head1.rotationPointZ = -8;
        }

        if (head1.rotateAngleX < 0)
        {
            head1.rotateAngleX += (2.094395F / 10);
        }
        else
        {
        ...
        if (head2.rotationPointY < 15)
        {
            head2.rotationPointY += 1;
        }
        else
        {
            head2.rotationPointY = 15;
        }

        if (head2.rotationPointZ > -12)
        {
            head2.rotationPointZ -= (9 / 10);
        }
        else
        {
            head2.rotationPointZ = -12;
        }

        if (head3_up.rotationPointY < 15)
        {
            head3_up.rotationPointY += 1;
        }
        else
        {
            head3_up.rotationPointY = 15;
        }

        if (head3_up.rotationPointZ > -20)
        {
            head3_up.rotationPointZ -= (9 / 10);
        }
        else
        {
            head3_up.rotationPointZ = -20;
        }

        if (head3_down.rotationPointY < 15)
        {
            head3_down.rotationPointY += 1;
        }
        else
        {
            head3_down.rotationPointY = 15;
        }

        if (head3_down.rotationPointZ > -19)
        {
            head3_down.rotationPointZ -= (9 / 10);
        }
        else
        {
            head3_down.rotationPointZ = -19;
        }
    }
    public void HeadUpper()
    {
        if (noumenon1.rotationPointZ > -4)
        {
            noumenon1.rotationPointZ -= 0.1;
        }
        else
        {
            noumenon1.rotationPointZ = -4;
        }

        if (noumenon1.rotateAngleX > -0.5235988F)
        {
            noumenon1.rotateAngleX -= (0.5235988F / 10);
        }
        else
        ...
        if (head1.rotationPointZ < -5)
        {
            head1.rotationPointZ += (3 / 10);
        }
        else
        {
            head1.rotationPointZ = -5;
        }

        if (head1.rotateAngleX > -2.094395F)
        {
            head1.rotateAngleX -= (2.094395F / 10);
        }
        else
        ...
        if (head2.rotationPointY > 5)
        {
            head2.rotationPointY -= 1;
        }
        else
        {
            head2.rotationPointY = 5;
        }

        if (head2.rotationPointZ < -3)
        {
            head2.rotationPointZ += (9 / 10);
        }
        else
        {
            head2.rotationPointZ = -3;
        }

        if (head3_up.rotationPointY > 5)
        {
            head3_up.rotationPointY -= 1;
        }
        else
        {
            head3_up.rotationPointY = 5;
        }

        if (head3_up.rotationPointZ < -11)
        {
            head3_up.rotationPointZ += (9 / 10);
        }
        else
        {
            head3_up.rotationPointZ = -11;
        }

        if (head3_down.rotationPointY > 5)
        {
            head3_down.rotationPointY -= 1;
        }
        else
        {
            head3_down.rotationPointY = 5;
        }

        if (head3_down.rotationPointZ < -10)
        {
            head3_down.rotationPointZ += (9 / 10);
        }
        else
        {
            head3_down.rotationPointZ = -10;
        }
    }
    public void OpenMouth(int Steps)
    {
        if (head3_down.rotateAngleX < 0.4363323F)
        {
            head3_down.rotateAngleX += (0.4363323F / Steps);
        }
        else
        {
            head3_down.rotateAngleX = 0.4363323F;
        }
    ...
    {
        if (head3_down.rotateAngleX > 0)
        {
            head3_down.rotateAngleX -= (0.4363323F / Steps);
        }
        else
        {
            head3_down.rotateAngleX = 0;
        }
