...
public class EntityMosasaurus extends EntityDinosaurce implements IWaterDino
{
    ...
    protected void attackEntity(Entity entity, float f)
    {
        ...
        else if ((double)f <= width * 1.6 && entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY < boundingBox.maxY)
        {
    ...
    private void Flee(Entity EscapeFrom, int range)
    {
        ...
        if (EscapeFrom.posX <= posX)
        {
            TargetX = (int)Math.round(posX) + DistanceX;
        }
        else
        {
            TargetX = (int)Math.round(posX) - DistanceX;
        ...
        if (EscapeFrom.posZ <= posZ)
        {
            TargetZ = (int)Math.round(posZ) + DistanceZ;
        }
        else
        {
            TargetZ = (int)Math.round(posZ) - DistanceZ;
    ...
    private boolean HuntForPrey(int range)
    {
        if ((this.getEntityToAttack() == null) || !(this.getEntityToAttack() instanceof EntityPlayer))
        {
            ...
            List list = worldObj.getEntitiesWithinAABB(EntityLiving.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(range, range * 2, range));
    ...
    public void HandleBoatSinking()
    {
        ...
        List list = worldObj.getEntitiesWithinAABB(EntityBoat.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(10, 4, 10));
        Iterator iterator = list.iterator();
    ...
    public EntityMosasaurus spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
