...
public class EntityNautilus extends EntityWaterMob
{
    ...
    {
        return mod_Fossil.EmptyShell.itemID;
    }
    ...
    protected void dropFewItems(boolean flag, int unusedi)
    {
        ...
        if (i <= 3)
        {
            dropItem(mod_Fossil.EmptyShell.itemID, 1);
        }
        else
        {
            dropItem(mod_Fossil.MagicConch.itemID, 1);
        }
    }
        ...
        if (this.BreedTick == 0)
        {
            ...
            if (!Extsit)
            {
                ...
                if (new Random().nextInt(100) < GroupAmount)
                {
                    ...
                    if (worldObj.checkIfAABBIsClear(entityliving.boundingBox) && worldObj.getCollidingBoundingBoxes(entityliving, entityliving.boundingBox).size() == 0 && worldObj.isAnyLiquid(entityliving.boundingBox))
                    {
