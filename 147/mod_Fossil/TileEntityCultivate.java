    ...
    implements IInventory, ISidedInventory
{
    ...
    public void updateEntity()
    {
        ...
        if (furnaceCookTime == 3001 && new Random().nextInt(100) <= 30)
        {
            ((BlockCultivate)mod_Fossil.blockcultivateIdle).onBlockRemovalLost(worldObj, xCoord, yCoord, zCoord, true);
        }
    }
    ...
    public void smeltItem()
    {
        ...
        else if (cultivateItemStacks[2].itemID == itemstack.itemID)
        {
    ...
    private int getItemBurnTime(ItemStack itemstack)
    {
        ...
        if (i == mod_Fossil.biofossil.itemID)
        {
            return 300;
        }

        if (i == mod_Fossil.ChickenSoup.itemID)
        {
            return 1000;
        }

        if (i == mod_Fossil.Ancientegg.itemID)
        {
            return 12000;
        }

        if (i == Item.porkRaw.itemID)
        {
            return 3000;
        }

        if (i == Item.fishRaw.itemID)
        {
            return 3000;
        }

        if (i == Item.beefRaw.itemID)
        {
            return 4000;
        }

        if (i == mod_Fossil.RawDinoMeat.itemID)
        {
            return 4000;
        }

        if (i == Item.chickenRaw.itemID)
        {
            return 1500;
        }

        if (i == Item.egg.itemID)
        {
            return 1000;
        }

        if (i == Item.slimeBall.itemID)
        {
            return 800;
        }

        if (i == Item.bucketMilk.itemID)
        {
            return 6000;
        }
        else
    ...
    {
        if (Itemstack.itemID == mod_Fossil.DNA.itemID)
        {
            return new ItemStack(mod_Fossil.Ancientegg, 1, Itemstack.getItemDamage());
        }
        ...
        if (Itemstack.itemID == mod_Fossil.AnimalDNA.itemID)
        {
            ...
            if (Tmp != 3)
            {
                return new ItemStack(mod_Fossil.EmbyoSyringe, 1, (Tmp > 3) ? Tmp - 1 : Tmp);
            }
            else
            {
