...
public class DinoAIGrowup extends EntityAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        if (AITarget.getDinoAge() > this.ageLimit)
        {
            return false;
        }
    ...
    public void updateTask()
    {
        ...
        if (AITarget.getHealth() < AITarget.getMaxHealth())
        {
            AITarget.heal(healingOnGrowing);
        }
        ...
        if (!AITarget.CheckSpace())
        {
            ...
            if (AITarget.getHealth() > healingOnGrowing)
            {
                AITarget.attackEntityFrom(DamageSource.generic, healingOnGrowing);
            }
            ...
            if (AITarget.isTamed())
            {
                AITarget.SendStatusMessage(EnumSituation.NoSpace, AITarget.SelfType);
            }
        }
    }
}
