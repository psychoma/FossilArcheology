...
public abstract class EntityDinosaurce extends EntityTameable implements IEntityAdditionalSpawnData
{
        ...
        if (this.getDinoAge() > 4)
        {
            ...
            if (this.BreedTick == 0)
            {
                ...
                if (new Random().nextInt(100) < GroupAmount)
                {
                    ...
                    if (worldObj.checkIfAABBIsClear(entityegg.boundingBox) && worldObj.getCollidingBoundingBoxes(entityegg, entityegg.boundingBox).size() == 0)
                    {
    ...
    {
        if (this.worldObj.getClosestPlayerToEntity(this, mod_Fossil.MESSAGE_DISTANCE) == null)
        {
            return;
        }
    ...
    public float GetDistanceWithTileEntity(TileEntity target)
    {
        if (target != null)
        {
            return (float)(Math.sqrt(Math.pow((double)(posX - target.xCoord), 2) + Math.pow((double)(posY - target.yCoord), 2) + Math.pow((double)(posZ - target.zCoord), 2)));
        }
        else
        {
            return -1;
    ...
    protected boolean modelizedInteract(EntityPlayer entityplayer)
    {
        ...
        final int STICKID = Item.stick.itemID;
        final int BONEID = Item.bone.itemID;
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound)
    {
        ...
        if (nbttagcompound.hasKey("isModelized"))
        {
            this.setModelized(nbttagcompound.getBoolean("isModelized"));
        }
