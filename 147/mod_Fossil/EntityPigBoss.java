...
public class EntityPigBoss extends EntityZombie
{
    ...
    public boolean getCanSpawnHere()
    {
        ...
        List list = worldObj.getEntitiesWithinAABB(EntityPigBoss.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(200D, 200D, 200D));
        result = (list.size() < 2);
        return result;
    }
    ...
    public void readEntityFromNBT(NBTTagCompound nbttagcompound)
    {
        ...
        if (nbttagcompound.hasKey("AttackMode"))
        {
            this.SetAttackMode(nbttagcompound.getInteger("AttackMode"));
        }
    }
    ...
    protected Entity findPlayerToAttack()
    {
        ...
        if (Target instanceof EntityPlayer && ((EntityPlayer) Target).getHealth() > 0)
        {
            ...
            if (!worldObj.provider.isHellWorld)
            {
                ((EntityPlayer) Target).addStat(mod_Fossil.PigbossOnEarth, 1);
            }
        }
    ...
    public void onLivingUpdate()
    {
        ...
        {
            if (this.texture != "/skull/PigBoss.png")
            {
                this.texture = "/skull/PigBoss.png";
            }
        ...
        {
            if (this.texture != "/skull/PigBoss_Charging.png")
            {
                this.texture = "/skull/PigBoss_Charging.png";
            }
        ...
        {
            List list = worldObj.getEntitiesWithinAABB(EntityPigZombie.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(16D, 4D, 16D));
        ...
        {
            List list = worldObj.getEntitiesWithinAABB(EntityPig.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(16D, 4D, 16D));

            if (list.size() >= 3)
            {
                this.TranferPigs(list);
            }
        }
    }
                ...
                else
                {
                    ...
                    if (!(PlayerWeapon.getItem() instanceof ItemBow) && !(PlayerWeapon.getItem() instanceof ItemSword))
                    {
                        ...
                        if (PlayerDistance > 6 && this.getAttackMode() != Ranged)
                        {
                            if (worldObj.provider.isHellWorld)
                            {
                                Mouth.SendSpeech(EnumPigBossSpeaks.LeartHere);
                            }
                            else
                            {
                                Mouth.SendSpeech(EnumPigBossSpeaks.LeartThere);
                            }
        ...
        if (!worldObj.provider.isHellWorld)
        {
            for (int i = (int)(Math.round(posX) - 1); i <= (int)(Math.round(posX) + 1); i++)
            {
                for (int k = (int)(Math.round(posZ) - 1); k <= (int)(Math.round(posZ) + 1); k++)
                {
                    ...
                    if (targetID == Block.stone.blockID || targetID == Block.cobblestone.blockID)
                    {
                        worldObj.setBlockWithNotify(i, (int)(Math.round(posY) - 1), k, Block.netherrack.blockID);
                    }

                    if (targetID == Block.dirt.blockID || targetID == Block.grass.blockID || targetID == Block.sand.blockID || targetID == Block.gravel.blockID)
                    {
                        worldObj.setBlockWithNotify(i, (int)(Math.round(posY) - 1), k, Block.slowSand.blockID);
                    }

                    if (targetID == Block.ice.blockID)
                    {
                        worldObj.setBlockWithNotify(i, (int)(Math.round(posY) - 1), k, Block.obsidian.blockID);
                    }

                    if (targetID == Block.blockClay.blockID)
                    {
                        worldObj.setBlockWithNotify(i, (int)(Math.round(posY) - 1), k, Block.glowStone.blockID);
                    }

                    if (i != Math.round(posX) && k != (Math.round(posZ)))
                    {
                        if (new Random().nextInt(2000) <= 1 && worldObj.isAirBlock(i, (int)(Math.round(posY)), k))
                        {
                            worldObj.setBlockWithNotify(i, (int)(Math.round(posY)), k, Block.fire.blockID);
                        }
                    }
            ...
            if (worldObj.getWorldTime() > 19000 || worldObj.getWorldTime() < 17000)
            {
            	worldObj.setWorldTime(17000);
            }
        }
    }
    ...
    {
        List list = worldObj.getEntitiesWithinAABB(EntityLiving.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(32D, 4D, 32D));
        ...
        if (!list.isEmpty())
        {
            ...
            for (int j = 0; j < list.size(); j++)
            {
                ...
                if (entity1 != this)
                {
                    entity1.knockBack(this, 0, d * 5, d1 * 5);
                }
                ...
                if (entity1 instanceof EntityPlayer)
                {
                    if (new Random().nextInt(1000) >= 950)
                    {
                        ((EntityPlayer)entity1).inventory.dropAllItems();
                    }
                }
            }
        }
